...
\usepackage{url}    % Resolve o erro das URLs nas referências
\usepackage{geometry}
\geometry{
  a4paper,
  left=3.0cm,
...
% Configuração elegante do Listings para exibir código Python
\lstset{
    language=Python,
...
Historicamente, o ambiente MATLAB foi muito utilizado para demonstrar o ganho dessas tecnologias através de comandos de abstração de GPU como \texttt{gpuArray} e \texttt{arrayfun}. No entanto, a indústria e a academia têm migrado massivamente para a linguagem **Python** devido à sua robustez, gratuidade e ao seu poderoso ecossistema de bibliotecas como **NumPy** (CPU vetorizada), **PyTorch** (computação em GPU e Deep Learning) e **Numba** (compilação JIT para kernels CUDA).

Este relatório documenta o desenvolvimento, a adaptação e a análise experimental de duas tarefas computacionais complexas: a Transformada Rápida de Fourier (FFT) tridimensional e o cálculo do Fractal do Conjunto de Mandelbrot. Os códigos originais propostos em MATLAB foram traduzidos e otimizados para Python e validados em uma GPU real de última geração, a **NVIDIA GeForce RTX 3060 Laptop GPU** (arquitetura Ampere, Compute Capability 8.6).
...
O script desenvolvido em [ex1.py](file:///c:/Users/mufas/OneDrive/Área de Trabalho/Faculdade/Arq_desemp/ex1.py) realiza a comparação sistemática do tempo de processamento de uma FFT unidimensional ao longo das colunas de uma matriz $3000 \times 3000$ (precisão double, totalizando **68,7 MB** de dados). O script mede de forma isolada: o processamento na CPU (NumPy), a computação pura na GPU (PyTorch), a execução na GPU incluindo a cópia bidirecional Host-Device-Host (PCIe) e a solução otimizada gerando a matriz randômica diretamente no device.
...
A validação física das implementações em Python foi executada na máquina de teste equipada com um processador Host Intel CPU e um acelerador de hardware **NVIDIA GeForce RTX 3060 Laptop GPU** (4.096 MB VRAM, 30 cores SM, 3.840 CUDA Cores, Compute Capability 8.6), rodando Python 3.12 com PyTorch 2.5.1+cu121.

### 4.1. Saídas do Terminal
...
### 4.2. Análise do Exercício 1: Impacto do Barramento PCIe
...
GPU Computação Pura & 0.035245 & 5.81x \\
GPU Com Transferência (PCIe) & 0.043008 & 4.76x \\
GPU Geração Direta (Otimizado) & 0.013602 & \textbf{15.05x} \\ \bottomrule
\end{tabular}
\end{table}

A partir desses dados, fica evidente que o **barramento PCIe é um limitador físico crucial** de desempenho. Ao incluir o tempo de alocação e transferência de dados entre o Host (RAM) e o Device (VRAM), o speedup da GPU despencou de **5.81x** para **4.76x**. 

A solução proposta, de gerar a matriz randômica diretamente no device via \texttt{torch.rand}, eliminou por completo o tráfego no gargalo de barramento PCIe, garantindo um novo tempo otimizado de **0,013602 segundos**, o que equivale a um speedup impressionante de **15,05x** mais rápido em relação à CPU.

### 4.3. Análise do Exercício 2: Mandelbrot e Superioridade de Kernels
...
CPU Vetorizada (NumPy) & 5.2300 & 7.90x \\
GPU Vetorizada (gpuArray / PyTorch) & 0.2277 & 180.40x \\
...
1. **O poder da vetorização na CPU:** Apenas eliminando o laço interpretado procedural nativo de Python e utilizando as operações vetorizadas de C do NumPy, obtivemos um ganho imediato de **7,9x** (reduzindo o tempo de 41 para 5,2 segundos). Isso ocorre porque as instruções vetoriais SIMD do processador são plenamente exploradas.
2. **A espetacular superioridade de Kernels Customizados (\texttt{arrayfun}):** A versão vetorizada na GPU via PyTorch levou **0,2277 segundos** (Speedup de **180,4x**). Em contrapartida, a versão que emula o \texttt{arrayfun} compilada JIT pelo Numba para a GPU executou o mesmo processamento em apenas **0,0185 segundos** (Speedup extraordinário de **2220,8x** em relação à CPU Serial e um ganho direto de **12,3x** sobre a própria GPU vetorizada).
...
* Comprovou-se a viabilidade e a excelente correspondência de tradução entre a plataforma MATLAB e o moderno ecossistema científico do Python (NumPy, PyTorch e Numba).
* Analisou-se quantitativamente o impacto nocivo do barramento físico PCIe e a eficácia de manter e gerar dados diretamente no acelerador.
* Demonstrou-se de forma prática a diferença fundamental entre aplicações limitadas por banda de memória (GPU vetorizada) e limitadas por processamento aritmético puro (Kernels customizados compilation JIT), comprovada pelo speedup colossal de **2220,8x** obtido no kernel customizado executado na NVIDIA GeForce RTX 3060.
...
\begin{enumerate}
    ...
    \item MathWorks. GPU Computing in MATLAB. Disponível em: \url{https://www.mathworks.com/help/distcomp/gpu-computing.html}.
